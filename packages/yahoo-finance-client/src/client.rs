...
use crate::error::YahooFinanceError;
use crate::models::{Chart, ChartResponse};
...
///
/// A decade of daily closes is ~200KB, and a forty-year history therefore under a megabyte, so
/// 4MiB is several times the largest thing this endpoint has reason to send while still bounding
/// what a malfunctioning or hostile upstream can do to the caller's memory. A request timeout
/// bounds how *long* a response may take, never how many bytes it may be — that is what this is
/// for. Callers with their own house number override it with
/// [`YahooFinanceClient::with_max_response_bytes`]; `sure-providers` does, so one const in that
...
/// The `User-Agent` this endpoint expects to see.
///
/// Wire knowledge, which is why it lives here rather than at the call site: the endpoint is the
/// one `yfinance` wraps, it is not a documented API, and it has historically answered a request
/// carrying reqwest's own agent — or none — with a challenge page instead of JSON. A browser-ish
/// string is what keeps it answering, and it is a property of talking to *this host* rather than
/// a decision any caller should have to know to repeat.
const USER_AGENT_VALUE: &str = "Mozilla/5.0";

/// A client for Yahoo Finance's chart endpoint.
///
...
impl YahooFinanceClient {
    ...
    /// Daily bars for one Yahoo symbol between two epoch seconds:
    /// `GET /{symbol}?period1=…&period2=…&interval=1d`.
    ///
    /// `symbol` is Yahoo's spelling (`MEL.NZ`, `VOO`), not a ticker and an exchange — mapping one
    /// onto the other is the caller's, see the crate docs. The window is epoch seconds for the
    /// same reason: which instants bound a trading day, and whether they need padding, is
    /// calendar work this crate has deliberately not been given the vocabulary for.
    ///
    /// `interval=1d` is fixed. The endpoint offers finer granularities and this crate does not:
    /// exposing one would be a new return shape (intraday bars are not daily closes) rather than
    /// a parameter, and nothing asks for it.
    ///
    /// The URL is built by concatenation rather than through `Url::query_pairs_mut`, and that is
    /// deliberate: the record/replay proxy in front of this in every test compares the query
    /// *verbatim*. Reshaping this string — a reordered parameter, an encoded one — silently
    /// invalidates every committed snapshot, and the suite would go on replaying answers to a
    /// request nothing makes any more. `sure-providers`' `tests/yahoo_finance.rs` asserts the
    /// exact origin-form URI for that reason.
    pub async fn chart(
        &self,
        symbol: &str,
        period1: i64,
        period2: i64,
    ) -> Result<Chart, YahooFinanceError> {
        let url = format!(
        ...
        // "some 4xx" and retried straight away — against this endpoint that is what turns a
        // throttle into an IP block on the whole machine. Both statuses that mean it are
        // covered: `429` is the unambiguous one; `503` is not — it is also what an ordinary
        // outage looks like — so a `503` only counts when it *names* a `Retry-After`, which is a
        // server saying "come back then" rather than "I am broken".
        ...
        {
            return Err(YahooFinanceError::RateLimited {
                status: status.as_u16(),
                retry_after,
            });
        }

        // A 404 is "this symbol does not exist", which is an ordinary answer rather than a
        // failure — and it is taken here, *before* the status check below, because the body it
        // arrives with (`{"chart":{"result":null,…}}`) is one the parser would reject. Reordering
        // these two turns every delisted holding into "no chart data returned".
        if status == reqwest::StatusCode::NOT_FOUND {
            return Err(YahooFinanceError::UnknownSymbol {
        ...
        if status.is_client_error() || status.is_server_error() {
            return Err(YahooFinanceError::Http {
                status: status.as_u16(),
                url,
            });
        }
        ...
        let parsed: ChartResponse = serde_json::from_slice(&body)
            .map_err(|error| YahooFinanceError::Deserialization { error, url })?;

        parsed
            .into_chart()
            .ok_or_else(|| YahooFinanceError::NoChartData {
                symbol: symbol.to_string(),
            })
    }
    ...
    async fn read_capped(
        ...
        url: &str,
    ) -> Result<Vec<u8>, YahooFinanceError> {
    ...
    fn enforce_cap(&self, bytes: u64, url: &str) -> Result<(), YahooFinanceError> {
        if bytes > self.max_response_bytes {
            return Err(YahooFinanceError::ResponseTooLarge {
...
///
/// Delay-seconds is what an upstream would actually send — this one sends neither, which is why
/// the caller has a default — and the HTTP-date form is parsed via `chrono`'s RFC 2822 reader,
/// which accepts IMF-fixdate (`Sun, 06 Nov 1994 08:49:37 GMT`), the same grammar with a fixed
/// offset. A date already in the past reads as zero rather than as an error: the upstream is
/// saying "now".
///
/// **`frankfurter-client` carries a copy of this function, deliberately.** Each client crate
...
mod tests {
    ...
    fn caps_at_the_ceiling_inclusive() {
        // A decade of closes (~200KB) and a body exactly at the ceiling both pass; one byte past
        // it, whether declared or accumulated, does not.
        let client = client();
        let url = "http://query1.finance.yahoo.com/v8/finance/chart/MEL.NZ?interval=1d";
        assert!(client.enforce_cap(200_000, url).is_ok());
    ...
    /// The message is the whole diagnostic: a scheduler log carries several adapters' failures,
    ...
    fn the_cap_error_names_the_host_and_the_size() {
        let err = client()
            .enforce_cap(
                ...
                "http://query1.finance.yahoo.com/v8/finance/chart/MEL.NZ?interval=1d",
            )
            .expect_err("twice the ceiling is over it")
            .to_string();
        assert!(err.contains("query1.finance.yahoo.com"), "{err}");
        assert!(
    ...
    /// The form an upstream would actually send. This one does not, but a `503` from whatever
    /// sits in front of it might.
    #[test]
    ...
    /// The case that matters most for this endpoint: it sends no `Retry-After` at all, and the
    /// `None` that produces has to be a real answer the caller can apply its own window to.
    #[test]
