...
sure-dal   ──►  sure-core          SQLite pool + migrations + every SQL query (per-entity
                                     repository modules: accounts, transactions, rules, …)
sure-providers ─► sure-core        TransactionProvider trait, registry, CSV importer
...
currencies, settings}`). Each module owns its row/request/response types (they derive
