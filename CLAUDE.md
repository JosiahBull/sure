...
- **A 0.x dependency's *minor* bump is breaking, and `.github/dependabot.yml` must say so.**
  ...
  update and auto-merge takes it at its word, and the `- dependency-name: "*"` semver-major
  ignore does not cover it. Already paid for: sqlx 0.8.6 → 0.9.0 (`bbc6ba9`) and tower-http
  0.6.11 → 0.7.0 (`cfb6491`) both landed on main stamped
  `update-type: version-update:semver-minor`, straight past that wildcard; and openapi-fetch
  0.16 → 0.17 collapses a tuple in the generated client and fails
  `pnpm --filter @sure/web check` (PR #9, closed; mechanism in
  `packages/client/package.json`). There is no setting for it, so every
  0.x direct dependency is listed by name under `update-types: ["version-update:semver-minor"]`
  in its own ecosystem's `ignore:` block and bumped by hand, exactly like a major; patch bumps
  still flow. A list is only as good as what keeps it current, which is
  `scripts/check-dependabot.sh` (the `Dependabot config` job in `checks.yml`; `--help` carries
  the reasoning) — it reads the requirement out of every dependency table in
  `Cargo.toml`/`packages/*/Cargo.toml` and both dependency fields of
  `package.json`/`packages/*/package.json`, and fails with the exact lines to paste. An entry
  counts only in the `updates:` block it sits in: a name listed under the npm ecosystem
  protects nothing on the cargo side.
